...
main(int argc, char *argv[]) {
    ...
    while ((o = getopt(argc, argv, "vf:")) != -1) {
        switch (o) {
            case 'f':
                filestem = copy_string(optarg);
                break;
            case 'v':
                verbosity++;
                break;
            case '?':
                printf("Option error in c4 to FOIL conversion\n");
                exit(1);
        }
    }
    ...
    while (get_next_name(fp, 1, 1)) {
        ...
        attributes[n_attributes] =
                (att_info) malloc(sizeof (struct _att_info));
        ...
        if (!strcmp(name, "ignore")) {
            ...
            attribute->n_values = 0;
            attribute->values =
                    (char **) malloc(attribute->discrete_n *
                    sizeof (char *));
            if (attribute->discrete_n
                    && (attribute->values == NULL)) {
                printf
                        ("Problem allocating space for values of %s\n",
                        attribute->name);
                exit(1);
            }
            get_type_name(attribute, fp);
            ...
            do {
                if (attribute->n_values
                        && !(attribute->n_values % 10))
                    attribute->values =
                        (char **) realloc((void *) attribute->
                        values,
                        (attribute->
                        n_values +
                        11) *
                        sizeof (char *));
                attribute->values[attribute->n_values++] =
                        copy_string(name);
                if (terminator != ',')
                    break;
        ...
        if (attribute->discrete)
            if (attribute->n_values)
                attribute->value_occurs_tr =
                    (int *) calloc((size_t) attribute->n_values,
                    sizeof (int));
            else
                attribute->value_occurs_tr =
                    (int *) calloc((size_t) attribute->
                    discrete_n, sizeof (int));
    }
    ...
    while (get_next_name(fp, 1, attributes[0]->discrete)) {
        if (n_cases && !(n_cases % 100))
            cases = (char ***) realloc((void *) cases,
                ...
                101) * sizeof (char **));
        cases[n_cases] =
                (char **) malloc((n_attributes + 1) * sizeof (char *));
    ...
    while (fp && get_next_name(fp, 1, attributes[0]->discrete)) {
        ...
        do {
            ...
            if (attribute->ignore) {
                ...
                testing_cases[n_testing_cases][i] =
                        attribute->values[j];
            } else { /* continuous attribute with value */
    ...
    for (i = 0; i < n_attributes; i++) {
        ...
        if (j == n_types) { /* New type */
            if (n_types && !(n_types % 10))
                types = (type_info *) realloc((void *) types,
                    ...
                    (type_info));
            types[n_types] =
                    (type_info) malloc(sizeof (struct _type_info));
            ...
            if (types[n_types]->discrete) {
                types[n_types]->values =
                        (char **) malloc(11 * sizeof (char *));
                types[n_types]->value_occurs_tr =
                        (int *) malloc(11 * sizeof (int *));
            } else {
        ...
        if (type->discrete != attribute->discrete) {
            printf
                    ("Type %s declared to be both discrete and continuous\n",
                    type->name);
            exit(1);
        }
    ...
    for (i = 0; i < n_types; i++) {
        ...
        for (j = i + 1; j < n_types; j++) {
            if (!type->discrete)
                continue;
            if ((int) strlen(types[i]->name) >
                    (int) strlen(types[j]->name)) {
                if (!strncmp
                        (types[i]->name, types[j]->name,
                        strlen(types[j]->name))) {
                    printf
                            ("Type name %s is prefix of type name %s\n",
    ...
    for (i = 0; i < n_cases; i++) {
        if (cases[i][n_attributes] != class_names[0])
            continue;
        ...
        for (j = 0; j < n_attributes; j++) {
            ...
            if (!cases[i][j]) { /* Missing Value */
                ...
                fprintf(tp, "%s%s", attribute->type_name,
                        cases[i][j]);
...
void get_type_name(att_info attribute, FILE * fp) {
    ...
    if (terminator == '|' && get_next_name(fp, 1, 1)
            && !strcmp(name, "type")
