...
OrigTTot, OrigFPos, /* ditto, ~R(A) */
OrigFTot, NowTPos, /* current pos tuples with extensions, R(A) */
NowTTot, NowFPos, /* ditto, ~R(A); NewFPos is identical */
NowFNeg, NowFTot, /* calculated for brevity at end */
NowMPos, /* pos tuples with missing values */
NowMTot, /* pos tuples with missing values */
NewTPos, /* pos tuples in new state, R(A) */
NewTTot, NNewVars, /* number of unbound vars in R(A) */
...
MaxFPos, PossibleCuts;

Var * OldVar, /* old var possibly equal to a new var */
*NewVarPosn, /* argument number of new var */
*ConstVarPosn; /* new vars with unchanging values */

Tuple FirstBinding; /* check for consts masquerading as new vars */

Boolean Allocate = true, NegatedLiteralOK, /* ~R(A) permissible  */
...
/*    ---------------  */ {
    printf("DEBUG: EvaluateLiteral(): Entering!\n");
    ...
    Verbose(3) {
        printf("DEBUG: Printing composed literal!\n");
        putchar('\t');
    ...
    if (R == GTCONST) {
        ...
        if ((Abandoned = TerminateScan(R, A)) &&
            ...
            Current.NPos - (NowFPos + NowMPos) <
            MinPos /* R(A) has insuff gain */) {
    ...
    printf("DEBUG: Coverage = %d!, Coverage\n");
    ...
    printf("DEBUG: EvaluateLiteral(): Leaving!\n");
}
...
/*    --------------  */ {
    ...
    if (Allocate) {
        ...
        PossibleVarComps =
                MAXARGS * MAXVARS + (MAXARGS * (MAXARGS - 1) / 2);
...
/*       -------------  */ {
    ...
    /* formulate the columns of matrix A in the LP problem */
    column col;
    col.R = R;
    int num_args = sizeof(A)/sizeof(*A);
    printf("DEBUG: num_args = %d\n", num_args);
    col.A = new Var[num_args]();
    memcpy(col.A, A, sizeof(A));
    col.entries = new int[lp.num_examples]();
    lp.problem.push_back(col);
    
    int first_idx = *Current.Tuples[0];
    printf("DEBUG: first index = %d\n", first_idx);
    ...
    for (TSP = Current.Tuples; Case = *TSP++;) {
        ...
        printf("%d: ", cur_idx);
        PrintTuple(Case, Current.MaxVar, Nil, true);
        ...
        if (FirstNegTuple && !Positive(Case)) {
            ...
            if (!PossibleT && !PossibleF) {
                ...
                Verbose(3) {
                    printf("  MDL prune %d,%d", OrigTPos,
                            OrigFPos);
                }
                printf("DEBUG: TerminateScan(): Leaving!\n");
                return true;
            }

            /*  Set thresholds now that NowTPos and NowFPos are known  */

            NewTNegThresh = (NNewVars
                    && BestLitGain <
                    1E-2 ? MAXTUPLES : NegThresh(NowTPos,
                    NewTPos));
        ...
        if (MissingValue(R, A, Case)) {
            ...
            printf("DEBUG: ");
            PrintComposedLiteral(R, true, A);
            printf(" missing value! should set to 0\n");
        } else
            ...
            printf("DEBUG: ");
            PrintComposedLiteral(R, true, A);
            printf(" satisfied! should set to 1\n");
            col.entries[cur_idx] = 1;
            ...
            if (Positive(Case)) {
                ...
                MaxCover =
                        OrigFPos + (Current.NOrigPos - OrigPos);
                if (MaxCover < MinSaveableCover) {
                    TermTest(PossibleF,
                            Current.NPos - (NowTPos +
                            NowMPos) <
                            MinPos - 1E-3);
                }
            } else {
            ...
            printf("DEBUG: ");
            PrintComposedLiteral(R, true, A);
            printf(" NOT satisfied! should set to 0\n");
            ...
            if (Positive(Case)) {
                ...
                MaxCover =
                        OrigTPos + (Current.NOrigPos - OrigPos);
                if (MaxCover < MinSaveableCover) {
                    TermTest(PossibleT,
                            Current.NPos - (NowFPos +
                            NowMPos) <
                            MinPos - 1E-3);
                }
            } else {
...
/*    ------------  */ {
    ...
    if (NConstVars) {
        if (!NowTTot) {
            memcpy(FirstBinding, Found[0],
                    (NArgs + 1) * sizeof (Const));
        }
...
/*    -------------  */ {
    ...
    while (NowTTot >= 1) {
        if (PosVal <= NegVal) {
            ...
            ThisVal = NegVal;
            NegVal =
                    (NowTTot > NowTPos ? FP(((*ScanNeg)[V])) : 1E30);
            ThisSign = 2;
        }
