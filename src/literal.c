...
/*    -----------  */ {
    printf("DEBUG: ExploreArgs(): Entering!\n");
    int MaxArgs = 1;
    Var V;
    ...
    {
        printf("DEBUG: ExploreArgs(): Leaving!\n");
        return;
    }

    if (Predefined(R)) {
        printf("DEBUG: R is predefined!\n");
        if (R == EQVAR) {
            ExploreEQVAR();
        } else if (R == EQCONST) {
            ExploreEQCONST();
    ...
    printf("DEBUG: Trying args!\n");
    TryArgs(R, 1, Current.MaxVar, 0, 0, 0, true, false);
    printf("DEBUG: Done Trying args!\n");
        ...
        ForEach(W, V + 1, Current.MaxVar) {
            ...
            if (Compatible[Variable[V]->Type][Variable[W]->Type]) {
                if (CountOnly) {
                    ...
                    Arg[2] = W;
                    EvaluateLiteral(EQVAR, Arg, EQVAR->Bits,
                            Nil);
                    EQVAR->NTried++;
                }
            }
...
/*       -------  */ {
    ...
    if (!CountOnly) {
        if (Ticks > TicksCheck) {
            if ((TimeSpent = CPUTime() - StartTime) > MAXTIME) {
                printf("DEBUG: TryArgs(): too much time spent! Leaving!\n");
                return false;
            } else if (TimeSpent > 0.001 * MAXTIME) {
                TicksCheck +=
                        0.01 * Ticks * MAXTIME / TimeSpent;
            } else {
                TicksCheck *= 10;
            }
        }
    ...
    if (TryMostGeneral &&
        ...
        if (AcceptableKey(R, CopyKey) &&
                (R != Target || RecOK || (RecOK = RecursiveCallOK(Arg)))) {
            if (CountOnly) {
                R->NTrialArgs++;
            } else if (!Repetitious(R, Arg)) {
                printf("DEBUG: going to evaluate current literal!\n");
                EvaluateLiteral(R, Arg, R->Bits, &Prune);
                printf("DEBUG: done evaluating current literal!\n");
                R->NTried++;
    ...
    for (V = 1; V <= MaxV && BestLitGain < MaxPossibleGain; V++) {
        ...
        ForEach(W, 1, This - 1) {
            UselessSameVar |= Arg[W] == V
                    && R->ArgNotEq[ArgPair(This, W)];
        }
        ...
        if (!TryArgs
                (R, This + 1, Max(HiVar, V), NewFreeVars, NewMaxDepth, Key,
                V <= HiVar, RecOK))
        {
            printf("DEBUG: TryArgs(): Leaving!\n");
            return false;
