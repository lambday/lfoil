        ...
        do {
            ...
            if (RecordTheoryConst) {
                if (T->NTheoryConsts % 10 == 0) {
                    Realloc(T->TheoryConst,
                            T->NTheoryConsts + 10, Const);
                }
        ...
        if (R->TypeRef[i]->Continuous) {
            if (!strcmp(Name, "?")) {
                ...
                if (FP(T[i]) == MISSING_FP) {
                    printf
                            ("An input continuous values is equal to the\n");
                    printf
                            ("magic number used to designate missing values.\n");
                    printf
                            ("Change the definition of MISSING_FP in defns.i\n");
...
/*    --------------------  */ {
    ...
    if (R->Neg) {
        ...
        if (CountDuplicates(NegCopy, N, 0, NNeg - 1)) {
            printf
                    ("    (warning: contains duplicate negative tuples)\n");
        }
...
/*     ---------  */ {
    ...
    ForEach(Arg, 1, N) {
        ...
        ForEach(Val, 1, MaxConst) {
            IX[Arg][Val] =
                    Next[Arg][Val] ? Alloc(Next[Arg][Val] + 1,
                    int) : Nil;
            Next[Arg][Val] = 0;
        }
    }
...
/*     ----------  */ {
    ...
    l = strlen(s) + 1;
    new_string = Alloc(l, char);
    memcpy(new_string, s, l);
