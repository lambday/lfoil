...
/*    ----  */ {
    ...
    if (sizeof (Const) != sizeof (float)) {
        printf
                ("Integers and floating point numbers are different sizes\n");
        printf
                ("Alter declaration of type Const (defns.i) and recompile\n");
        exit(1);
    }

    printf("FOIL 6.4   [January 1996]\n--------\n");
    printf("DEBUG: Testing C++ support!\n");
    std::vector<int> vec;
    for (int i = 0; i < 3; ++i)
    {
        vec.push_back(i);
    ...
    /*  Read input  */
    printf("DEBUG: Reading types!\n");
    ReadTypes();
    printf("DEBUG: Done reading types!\n");

    printf("DEBUG: Checking type compatibility!\n");
    CheckTypeCompatibility();
    printf("DEBUG: Done checking type compatibility!\n");

    printf("DEBUG: Reading relations!\n");
    ReadRelations();
    printf("DEBUG: Done reading relations!\n");

    /*  Initialise all global variables that depend on parameters  */

    printf("DEBUG: Initializing global vars depending on params!\n");
    ...
    printf("DEBUG: Done initializing global vars depending on params!\n");
    ...
    printf("DEBUG: Finding plausible ordering constants!\n");
    OrderConstants();
    printf("DEBUG: Done finding plausible ordering constants!\n");
    ...
    ForEach(i, 0, MaxRel) {
        ...
        if (R->PossibleTarget) {
            printf("DEBUG: Finding definitions and evaluating literals!\n");
            FindDefinition(R);
            printf("DEBUG: Done finding definitions and evaluating literals!\n");
        }
    }
        ...
        while (Case = ReadTuple(R)) {
            ...
            if (Interpret(R, Case) != (PlusOrMinus == '+')) {
                ...
                Verbose(1) {
                    ...
                    PrintTuple(Case, R->Arity, R->TypeRef,
                            false);
                }
                Errors++;
            }
